...
#pragma FCUDA TRANSFER cores=[1] type=burst dir=[1] pointer=[od] size=[num_threads2] begin name=writeod2 unroll=1 mpart=1 array_split=[dataDiff1]
    od[g_wpos + tid] = dataDiff2[tid + num_threads2];
#pragma FCUDA TRANSFER cores=[1] type=burst dir=[1] pointer=[od] size=[num_threads2] end name=writeod2 unroll=1 mpart=1 array_split=[dataDiff1]
  }
}
