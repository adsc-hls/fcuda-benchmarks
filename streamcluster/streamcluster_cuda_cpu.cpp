...
// GLOBAL
static DATATYPE *switch_membership;		//whether to switch membership in pgain
static DATATYPE *is_center;				//whether a point is a center
static int  *center_table;			//index table of centers
static int nproc; 					//# of threads
DATATYPE isCoordChanged;
...
float pspeedy(Points *points, float z, long *kcenter, int pid, pthread_barrier_t* barrier)
{
  ...
  if( pid != 0 ) { // we are not the master threads. we wait until a center is opened.
    ...
    for(i = 1; i < points->num; i++ )  {
      DATATYPE to_open = ((float)lrand48()/(float)INT_MAX)<(points->p[i].cost/z);
      if( to_open )  {
        (*kcenter)++;
#ifdef ENABLE_THREADS
        pthread_mutex_lock(&mutex);
...
void copycenters(Points *points, Points* centers, long* centerIDs, long offset)
{
  ...
  DATATYPE *is_a_median = (DATATYPE *) calloc(points->num, sizeof(DATATYPE));
...
int compare_outcenterIDs( Points* centers, long* centerIDs, char* gold_file ) {
  ...
  for( int i = 0; i < centers->num; i++ ) {
    if( is_a_median[i] ) {
      fscanf(fp, "%u\n", &gold_centerIDs);
      if (gold_centerIDs != centerIDs[i]) {
        printf("centerIDs mismatched at index %d: %u %u\n", i, gold_centerIDs, centerIDs[i]);
        return 0;
      }
      fscanf(fp, "%lf\n", &gold_weight);
      if (gold_weight - centers->p[i].weight < -EPSILON || gold_weight - centers->p[i].weight  > EPSILON) {
        printf("weight mismatched at index %d: %lf %lf\n", i, gold_weight, centers->p[i].weight);
        return 0;
      }
      for( int k = 0; k < centers->dim; k++ ) {
        fscanf(fp, "%lf\n", &gold_coord);
        if (gold_coord - centers->p[i].coord[k] < -EPSILON || gold_coord - centers->p[i].coord[k] > EPSILON) {
          printf("coord mismatched at index %d: %lf %lf\n", i, gold_coord, centers->p[i].coord[k]);
          return 0;
        }
      }
    }
    ...
    long chunksize, long centersize, char* outfile )
{
  ...
  while(1) {
    ...
    switch_membership = (DATATYPE*)malloc(points.num*sizeof(DATATYPE));
    is_center = (DATATYPE*)calloc(points.num,sizeof(DATATYPE));
  ...
  switch_membership = (DATATYPE*)malloc(centers.num*sizeof(DATATYPE));
  is_center = (DATATYPE*)calloc(centers.num,sizeof(DATATYPE));
