...
//=======================================
__global__ void
kernel_compute_cost(int num, int dim, int x, Point *p, int K, int stride,
float *coord_d, float *work_mem_d, int *center_table_d, int *switch_membership_d)
{
...
void allocDevMem(int num, int dim)
{
  ...
  //CUDA_SAFE_CALL( cudaMalloc((void**) &switch_membership_d, num * sizeof(int))  );
  switch_membership_d = (int*)malloc(sizeof(int)*num);
...
//=======================================
float pgain( int x, Points *points, float z, int *numcenters, int kmax, int *is_center, int *center_table, int *switch_membership, int isCoordChanged,
    double *serial_t, double *cpu_to_gpu_t, double *gpu_to_cpu_t, double *alloc_t, double *kernel_t, double *free_t)
{
  static int fcuda = 0;
  // add a printf here to make sure it is still running!
  printf("iter %d\n", fcuda);
  fcuda++;
  ...
  //CUDA_SAFE_CALL( cudaMemset((void*) switch_membership_d, 0,			num * sizeof(int))  );
  memset(switch_membership_d, 0, num * sizeof(int));
  ...
  //int *assign_h = (int*)malloc(num * sizeof(int));
  long long *assign_h = (long long*)malloc(num * sizeof(long long));
  ...
  //printf("%d %d %d %d\n", num, num_blocks, num_blocks_x, num_blocks_y);
  ...
  XFcuda_SetNum(&xcore, num);
  XFcuda_SetDim(&xcore, dim);
  XFcuda_SetX(&xcore, x);
  XFcuda_SetK(&xcore, K);
  XFcuda_SetStride(&xcore, stride);
  XFcuda_SetGriddim_x(&xcore, grid_size.x);
  XFcuda_SetGriddim_y(&xcore, grid_size.y);
  //XFcuda_SetGriddim_z(&xcore, grid_size.z);
  XFcuda_SetBlockdim_x(&xcore, THREADS_PER_BLOCK);
  //XFcuda_SetBlockdim_y(&xcore, 1);
  //XFcuda_SetBlockdim_z(&xcore, 1);
  XFcuda_SetWeight_addr(&xcore, (u32)weight_h / sizeof(float));
  //XFcuda_SetAssign_addr(&xcore, (u32)assign_h / sizeof(int));
  XFcuda_SetAssign_addr(&xcore, (u32)assign_h / sizeof(long long));
  XFcuda_SetCost_addr(&xcore, (u32)cost_h / sizeof(float));
  XFcuda_SetCoord_d_addr(&xcore, (u32)coord_d / sizeof(float));
  XFcuda_SetWork_mem_d_addr(&xcore, (u32)work_mem_d / sizeof(float));
  XFcuda_SetCenter_table_d_addr(&xcore, (u32)center_table_d / sizeof(int));
  XFcuda_SetSwitch_membership_d_addr(&xcore, (u32)switch_membership_d / sizeof(int));
  ...
  //CUDA_SAFE_CALL( cudaMemcpy(switch_membership, switch_membership_d,	 num * sizeof(int),  cudaMemcpyDeviceToHost) );
  memcpy(switch_membership, switch_membership_d, num * sizeof(int));
#ifdef CUDATIME
  //cudaEventRecord(stop,0);
  //cudaEventSynchronize(stop);
  //cudaEventElapsedTime(&tmp_t, start, stop);
