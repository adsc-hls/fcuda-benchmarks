...
//float pspeedy(Points *points, float z, int *kcenter, int pid, pthread_barrier_t* barrier)
float pspeedy(Points *points, float z, int *kcenter, int pid)
{
#ifdef PROFILE
  double t1 = gettime();
#endif
...
double pgain1(long long x, Points *points, double z, int *numcenters, int pid)
{
  ...
  //my block
  long long bsize = points->num/nproc;
  long long k1 = bsize * pid;
  long long k2 = k1 + bsize;
  if( pid == nproc-1 ) k2 = points->num;
  ...
  memset(switch_membership + k1, 0, (k2-k1)*sizeof(int));
...
//	  float z, int *k, int kmax, float cost, int iter, float e,
//	  int pid, pthread_barrier_t* barrier)
float pFL(Points *points, int *feasible, int numfeasible,
    float z, int *k, int kmax, float cost, int iter, float e, int pid)
{
#ifdef ENABLE_THREADS
  pthread_barrier_wait(barrier);
#endif
  ...
  while (change/cost > 1.0*e) {
    ...
    for (i=0;i<iter;i++) {
      ...
      //change += pgain1(feasible[x], points, z, k, pid);
    }
    ...
    int pid)
{
  ...
  while(1) {
    ...
    //	       z, &k, kmax, cost, (int)(ITER*kmax*log((float)kmax)), 0.1, pid, barrier);
    cost = pFL(points, feasible, numfeasible,
        z, &k, kmax, cost, (int)(ITER*kmax*log((float)kmax)), 0.1, pid);
    ...
    if (((k <= (1.1)*kmax)&&(k >= (0.9)*kmin))||
      ...
      //      cost = pFL(points, feasible, numfeasible,
      //		 z, &k, kmax, cost, (int)(ITER*kmax*log((float)kmax)), 0.001, pid, barrier);
      cost = pFL(points, feasible, numfeasible,
          z, &k, kmax, cost, (int)(ITER*kmax*log((float)kmax)), 0.001, pid);
    }
...
void localSearch( Points* points, int kmin, int kmax, int* kfinal ) {
#ifdef PROFILE
  double t1 = gettime();
#endif
...
#define EPSILON 0.001
  int compare_outcenterIDs( Points* centers, int* centerIDs, char* gold_file ) {
    /*
    ...
    //unsigned int gold_centerIDs;
    //double gold_weight, gold_coord;
    int scan = 0;
    for( int i = 0; i < centers->num; i++ ) {
      if( is_a_median[i] ) {
        //fscanf(fp, "%u\n", &gold_centerIDs);
        if ((int)gold_output[scan] != centerIDs[i]) {
          printf("centerIDs mismatched at index %d: %u %u\n", i, (int)gold_output[scan], centerIDs[i]);
          //return 0;
        }
        scan++;
        //fscanf(fp, "%lf\n", &gold_weight);
        if (gold_output[scan] - centers->p[i].weight < -EPSILON || gold_output[scan] - centers->p[i].weight  > EPSILON) {
          printf("weight mismatched at index %d: %lf %lf\n", i, gold_output[scan], centers->p[i].weight);
          //return 0;
        }
        for( int k = 0; k < centers->dim; k++ ) {
          scan++;
          //fscanf(fp, "%lf\n", &gold_coord);
          if (gold_output[scan] - centers->p[i].coord[k] < -EPSILON || gold_output[scan] - centers->p[i].coord[k] > EPSILON) {
            printf("coord mismatched at index %d: %lf %lf\n", i, gold_output[scan], centers->p[i].coord[k]);
            //return 0;
          }
        }
        scan++;
  ...
  void streamCluster( PStream* stream,
      int kmin, int kmax, int dim,
      int chunksize, int centersize, char* outfile )
  {
    float* block = (float*)malloc( chunksize*dim*sizeof(float) );
    if (block == NULL) {
      printf("Cannot allocate memory to block\n");
      exit(1);
    }
    float* centerBlock = (float*)malloc(centersize*dim*sizeof(float) );
    if (centerBlock == NULL) {
      printf("Cannot allocate memory to centerBlock\n");
      exit(1);
    }
    int* centerIDs = (int*)malloc(centersize*dim*sizeof(int));
    if (centerIDs == NULL) {
      printf("Cannot allocate memory to centerIDs\n");
      exit(1);
    }
    ...
    if (points.p == NULL) {
      printf("Cannot allocate memory to p\n");
      exit(1);
    }
    ...
    if (centers.p == NULL) {
      printf("Cannot allocate memory to centers.p\n");
      exit(1);
    }
    centers.num = 0;
    ...
    while(1) {
      ...
      if (switch_membership == NULL) {
        printf("Cannot allocate memory to switch_membership\n");
        exit(1);
      }
      is_center = (int*)calloc(points.num,sizeof(int));
      if (is_center == NULL) {
        printf("Cannot allocate memory to is_center\n");
        exit(1);
      }
      center_table = (int*)malloc(points.num*sizeof(int));
      if (center_table == NULL) {
        printf("Cannot allocate memory to center_table\n");
        exit(1);
      }
...
#ifdef VERIFY
    ///*
    int pass = compare_outcenterIDs(&centers, centerIDs, "cuda/gold_output.txt");
    if (pass)
      printf("PASSED.\n");
    else
      printf("FAILED.\n");
  ...
  int main(int argc, char **argv)
  {
    ...
    int kmin, kmax, n, chunksize, clustersize;
    int dim;
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
    ...
    dim = 128; //atoi(argv[3]);
    n = 4096; //atoi(argv[4]);
    chunksize = 4096; //atoi(argv[5]);
    clustersize = 100; //atoi(argv[6]);
