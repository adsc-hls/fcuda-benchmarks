...
//#pragma FCUDA TRANSFER cores=[1] type=burst dir=[0|0] pointer=[temp_src|power] size=[16|16] begin name=fetch unroll=1 mpart=1 array_split=[temp_t]
...
//#pragma FCUDA TRANSFER cores=[1] type=burst dir=[0|0] pointer=[temp_src|power] size=[16|16] end name=fetch unroll=1 mpart=1 array_split=[temp_t]
	}
        	...
        	if(IN_RANGE(tx, i+1, BLOCK_SIZE-i-2) &&  \
            	   ...
            	   IN_RANGE(ty, validYmin, validYmax)) {
            		...
            		temp_t[ty][tx] =   temp_on_cuda[ty][tx] + step_div_Cap * (power_on_cuda[ty][tx] +
                         	 	(temp_on_cuda[S][tx] + temp_on_cuda[N][tx] - 2*temp_on_cuda[ty][tx]) * Ry_1 +
                             		(temp_on_cuda[ty][E] + temp_on_cuda[ty][W] - 2*temp_on_cuda[ty][tx]) * Rx_1 +
