...
#include <sys/time.h>
#include <time.h>
//#include "fcuda_generated_kernel_naive.c"
...
void videoSequence(int * I, int IszX, int IszY, int Nfr, int * seed){
  ...
  memset(newMatrix, 0, IszX * IszY * Nfr * sizeof(int));
...
int particleFilter(int * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles){
  ...
  for(k = 1; k < Nfr; k++){
    ...
    kernel(arrayX_GPU, arrayY_GPU, CDF_GPU, u_GPU, xj_GPU, yj_GPU, Nparticles, num_blocks, threads_per_block, 1, 0);
...
int main(int argc, char * argv[]){
  ...
  memset(I, 0, IszX * IszY * Nfr * sizeof(int));
  long long start = get_time();
  //call video sequence
