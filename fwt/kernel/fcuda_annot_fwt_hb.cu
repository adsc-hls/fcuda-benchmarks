...
#pragma FCUDA PORTMERGE remove_port_name=d_Input port_id=0
#pragma FCUDA PORTMERGE remove_port_name=d_Output port_id=0
//note: this kernel does not allow array partitioning as it will lead to incorrect result
__global__ void fwtBatch1Kernel(DATATYPE *d_Output, DATATYPE *d_Input, int log2N)
{
    	const int    N = 1 << log2N;
    	const int base = blockIdx.x << log2N;
    	__shared__ DATATYPE s_data[ELEMENTARY_SIZE];
	__shared__ DATATYPE s_data1[ELEMENTARY_SIZE];
...
#pragma FCUDA COMPUTE cores=[1] begin name=vec_blk unroll=1
		...
		//__syncthreads(); // FCUDA: avoid creating ThreadLoop here to make the execution correctly.
        	DATATYPE D0 = s_data[i0];
        	DATATYPE D1 = s_data[i1];
       	 	DATATYPE D2 = s_data[i2];
        	DATATYPE D3 = s_data[i3];
	...
	if(log2N & 1) {
		...
		int pos2;
    		for(pos2 = threadIdx.x+0; pos2 < N / 2; pos2 += blockDim.x) {
        		...
        		DATATYPE D4 = s_data[i4];
        		DATATYPE D5 = s_data[i5];
        		s_data[i4] = D4 + D5;
        		s_data[i5] = D4 - D5;
...
#pragma FCUDA PORTMERGE remove_port_name=d_Input port_id=0
#pragma FCUDA PORTMERGE remove_port_name=d_Output port_id=0
__global__ void fwtBatch2Kernel(DATATYPE *d_Output, DATATYPE *d_Input, int stride)
{
    	const int pos = blockIdx.x * blockDim.x;
    	__shared__ DATATYPE bram_d0[THREAD_N];
    	__shared__ DATATYPE bram_d1[THREAD_N];
    	__shared__ DATATYPE bram_d2[THREAD_N];
    	__shared__ DATATYPE bram_d3[THREAD_N];

	__shared__ DATATYPE bram_d01[THREAD_N];
	__shared__ DATATYPE bram_d11[THREAD_N];
	__shared__ DATATYPE bram_d21[THREAD_N];
	__shared__ DATATYPE bram_d31[THREAD_N];
...
#pragma FCUDA COMPUTE cores=[1] begin name=vec_blk array_split=[bram_d0|bram_d1|bram_d2|bram_d3] unroll=1 mpart=1
    	...
    	DATATYPE D0 = bram_d0[threadIdx.x];
    	DATATYPE D1 = bram_d1[threadIdx.x];
    	DATATYPE D2 = bram_d2[threadIdx.x];
    	DATATYPE D3 = bram_d3[threadIdx.x];
