...
runTest( int argc, char** argv)
{
	...
	*/
	...
	Status = XFcuda1_Initialize(&xcore1, 0);
	if (Status != XST_SUCCESS) {
		xil_printf("Initialization kernel 1 failed %d\r\n", Status);
		exit(1);
	}

	Status = XFcuda2_Initialize(&xcore2, 0);
	if (Status != XST_SUCCESS) {
		xil_printf("Initialization kernel 2 failed %d\r\n", Status);
		exit(1);
	}
#endif
...
#ifdef HW
	XFcuda1_SetCols(&xcore1, cols);
	XFcuda1_SetRows(&xcore1, rows);
	XFcuda1_SetQ0sqr(&xcore1, q0sqr);
	XFcuda1_SetGriddim_x(&xcore1, dimGrid.x);
	XFcuda1_SetGriddim_y(&xcore1, dimGrid.y);
	XFcuda1_SetGriddim_z(&xcore1, dimGrid.z);
	XFcuda1_SetBlockdim_x(&xcore1, dimBlock.x);
	XFcuda1_SetBlockdim_y(&xcore1, dimBlock.y);
	XFcuda1_SetBlockdim_z(&xcore1, dimBlock.z);
	XFcuda1_SetJ_cuda_addr(&xcore1, (u32)J_cuda / sizeof(double));
	XFcuda1_SetC_cuda_addr(&xcore1, (u32)C_cuda / sizeof(double));
	XFcuda1_SetE_c_addr(&xcore1, (u32)E_C / sizeof(double));
	XFcuda1_SetW_c_addr(&xcore1, (u32)W_C / sizeof(double));
	XFcuda1_SetN_c_addr(&xcore1, (u32)N_C / sizeof(double));
	XFcuda1_SetS_c_addr(&xcore1, (u32)S_C / sizeof(double));
	
	XFcuda2_SetCols(&xcore2, cols);
	XFcuda2_SetRows(&xcore2, rows);
	XFcuda2_SetQ0sqr(&xcore2, q0sqr);
	XFcuda2_SetLambda(&xcore2, lambda);
	XFcuda2_SetGriddim_x(&xcore2, dimGrid.x);
	XFcuda2_SetGriddim_y(&xcore2, dimGrid.y);
	XFcuda2_SetGriddim_z(&xcore2, dimGrid.z);
	XFcuda2_SetBlockdim_x(&xcore2, dimBlock.x);
	XFcuda2_SetBlockdim_y(&xcore2, dimBlock.y);
	XFcuda2_SetBlockdim_z(&xcore2, dimBlock.z);
	XFcuda2_SetJ_cuda_addr(&xcore2, (u32)J_cuda / sizeof(double));
	XFcuda2_SetC_cuda_addr(&xcore2, (u32)C_cuda / sizeof(double));
	XFcuda2_SetE_c_addr(&xcore2, (u32)E_C / sizeof(double));
	XFcuda2_SetW_c_addr(&xcore2, (u32)W_C / sizeof(double));
	XFcuda2_SetN_c_addr(&xcore2, (u32)N_C / sizeof(double));
	XFcuda2_SetS_c_addr(&xcore2, (u32)S_C / sizeof(double));

	//Xil_DCacheDisable();
	Xil_DCacheFlush();
	//init_timer(timer_ctrl, timer_counter_l, timer_counter_h);
	//start_timer(timer_ctrl);
	XFcuda1_Start(&xcore1);
	//XFcuda2_Start(&xcore2);

	while (!XFcuda1_IsDone(&xcore1));

	XFcuda2_Start(&xcore2);

	while (!XFcuda2_IsDone(&xcore2));
